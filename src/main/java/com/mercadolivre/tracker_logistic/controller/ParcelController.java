...
public class ParcelController {
    ...
    @GetMapping("/{parcelId}")
    public ResponseEntity<ParcelEntity> getParcelById(@PathVariable("parcelId") UUID id, @RequestParam(defaultValue = "true") boolean showEvents) {
    ...
    @GetMapping
    public ResponseEntity<Page<ParcelEntity>> getParcelsByFiter(@RequestParam(required = false) String sender, @RequestParam(required = false) String recipient, Pageable pageable) {

        Page<ParcelEntity> parcels = parcelQueryService.getParcelsByFilter(sender, recipient, pageable);
